...
public class InvertBinaryTreeSolution2Test {
    @Test
    public void TestCase1() {
        TestBinaryTree tree = new TestBinaryTree(1);
        tree.insert(new int[] {2, 3, 4, 5, 6, 7, 8, 9}, 0);
        InvertBinaryTreeSolution2.invertBinaryTree(tree);
    ...
    private boolean compareBT(InvertBinaryTreeSolution2.BinaryTree tree1, InvertedBinaryTree tree2) {
    ...
    class TestBinaryTree extends InvertBinaryTreeSolution2.BinaryTree {
        ...
        public void insert(int[] values, int i) {
            ...
            ArrayDeque<InvertBinaryTreeSolution2.BinaryTree> queue = new ArrayDeque<InvertBinaryTreeSolution2.BinaryTree>();
            queue.addLast(this);
            while (queue.size() > 0) {
                InvertBinaryTreeSolution2.BinaryTree current = queue.pollFirst();
                if (current.left == null) {
                    current.left = new InvertBinaryTreeSolution2.BinaryTree(values[i]);
                    break;
                }
                queue.addLast(current.left);
                if (current.right == null) {
                    current.right = new InvertBinaryTreeSolution2.BinaryTree(values[i]);
                    break;
                }
                queue.addLast(current.right);
            }
